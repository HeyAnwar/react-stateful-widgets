...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    return programmers[featuredPro - 1].name
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: featuredPro === null ? 'royalblue' : 'gold' // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setFeaturedPro(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
